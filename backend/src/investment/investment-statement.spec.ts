...
describe('buildStatementData', () => {
  it('regroupe les rendements par mois, hors des mouvements du wallet', () => {
    ...
    expect(data.yields[0]).toMatchObject({ month: '2026-08', ref: 'RDT-202608', count: 2 });
  ...
  it('numérote chaque opération et tient un solde courant du wallet', () => {
    const data = buildStatementData(makeRows(), [], 3, NOW);
    expect(data.operations.map((o) => o.kind)).toEqual(['DEPOSIT', 'PLACEMENT', 'WITHDRAWAL']);
    expect(data.operations.every((o) => /^OP-[0-9A-F]{8}$/.test(o.ref))).toBe(true);
    expect(data.operations.map((o) => o.balanceAfter.toNumber())).toEqual([1000, 100, 500]);
  ...
  it('rapproche le wallet : ouverture + apports + encaissements - placements - virements = clôture', () => {
    ...
    expect(
      w.opening.plus(w.contributions).plus(w.proceeds).minus(w.placements).minus(w.transfersOut).toNumber(),
    ).toBe(w.closing.toNumber());
  });

  it('rapproche le portefeuille : ouverture + placé - retiré + rendement = clôture', () => {
    const { openingValue, deposits, withdrawals, yieldNet, closingValue, returnPct } = buildStatementData(
      makeRows(),
      [],
      3,
      NOW,
    ...
    expect(openingValue.plus(deposits).minus(withdrawals).plus(yieldNet).toNumber()).toBe(closingValue.toNumber());
    expect(returnPct).toBeCloseTo((12 / 900) * 100, 6);
  });

  it("reprend le solde du wallet d'avant la période comme solde d'ouverture", () => {
    const rows = [
      row('DEPOSIT', 700, '2026-06-01T10:00:00', { creditTarget: 'INVESTMENT' }),
  ...
  it('ne garde que les positions actives ou touchées par la période', () => {
    ...
    const mk = (id: string, status: 'ACTIVE' | 'CLOSED', closedAt: Date | null): StatementPositionRow => ({
      id,
      kind: 'BASKET',
      key: 'RWA_STRATEGY',
      status,
    ...
    const data = buildStatementData([], [mk('a', 'ACTIVE', null), mk('b', 'CLOSED', old)], 3, NOW);
...
describe('statementFingerprint', () => {
  it('est stable pour les mêmes données et change si un montant change', () => {
    const rows = makeRows();
    const a = statementFingerprint(buildStatementData(rows, [], 3, NOW), 'HV-TEST');
    const b = statementFingerprint(buildStatementData(rows, [], 3, NOW), 'HV-TEST');
    expect(a).toMatch(/^[0-9A-F]{64}$/);
    expect(a).toBe(b);
    const tampered = rows.map((r) => (r.type === 'INVESTMENT_DEPOSIT' ? { ...r, amount: 901 } : r));
    expect(statementFingerprint(buildStatementData(tampered, [], 3, NOW), 'HV-TEST')).not.toBe(a);
  });
});
