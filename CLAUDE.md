...
- **Projection de remboursement flexible** (`RepaymentProjection`) : rendement annuel estimé (calculé automatiquement à partir de la performance réelle sur 12 mois des actifs générateurs de rendement, cf. `useEstimatedYield`, modifiable) + apport externe mensuel (suggéré automatiquement pour rembourser dans les 12 mois contractuels, modifiable à la baisse pour explorer un remboursement plus lent).  
- **Simulation à échéances fixes** (`InstallmentSchedule`) : mensualité fixe façon amortissement classique sur une durée choisie (6/12/24/36 mois), réduite par le même rendement estimé automatique, plafonné à 60 % de la mensualité (même plafond que §2A). Indique aussi la date du premier remboursement (mois suivant l'émission) avec le rappel que, s'agissant d'une ligne de crédit et non d'un prêt versé en une fois, aucun remboursement n'est réellement dû tant que le crédit n'est pas utilisé.  
